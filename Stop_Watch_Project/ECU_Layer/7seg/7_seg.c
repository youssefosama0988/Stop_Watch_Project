...
Std_ReturnType seven_segement_intialize(const segment_t *seg){
    ...
    if(NULL == seg){
        ...
        ret = gpio_pin_direction_initialize_write(&(seg->segment_pins[SEGEMENT_PIN0]));
        ret = gpio_pin_direction_initialize_write(&(seg->segment_pins[SEGEMENT_PIN1]));
        ret = gpio_pin_direction_initialize_write(&(seg->segment_pins[SEGEMENT_PIN2]));
        ret = gpio_pin_direction_initialize_write(&(seg->segment_pins[SEGEMENT_PIN3]));
    }
    return ret;
}
